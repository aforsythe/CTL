...
float[3] tonemapAndCompress_inv(float inputJMh[3],
                                ODTParams PARAMS,
                                float REACH_GAMUT_TABLE[][3],
                                float REACHM_TABLE[])
{
    // Recover pre-tonescale J from the tonemapped J by reversing the
    // forward luminance chain. Note: despite its comment, `tonescale_inv`
    // expects the dimensionless h = luminance/n_r — tonescale_fwd returns
    // `h * n_r`, so the inverse expects you to divide out n_r first.
    TSParams TSPARAMS = init_TSParams(PARAMS.peakLuminance);
    float luminanceTS = Hellwig_J_to_Y(inputJMh[0]);
    float linear = tonescale_inv(luminanceTS / TSPARAMS.n_r, TSPARAMS);
    float originalJ = Y_to_Hellwig_J(linear * referenceLuminance);

    // Un-compress M using the recovered pre-tonescale J as origJ.
    float untonemappedM = chromaCompression(inputJMh,
                                            originalJ,
                                            PARAMS,
                                            REACH_GAMUT_TABLE,
                                            REACHM_TABLE,
    ...
    float outputJMh[3] = {originalJ, untonemappedM, inputJMh[2]};
    return outputJMh;
}
...
// ACES v2 output-transform pipeline: aces → CAM16 JMh → tonemap +
// chroma compression → gamut map → display XYZ (Rec.709 @ 100 nits).
//
// All four per-hue tables (REACH_GAMUT_TABLE, REACHM_TABLE,
// GAMUT_CUSP_TABLE, GAMUT_TOP_GAMMA) are computed once at module load
// by the SIMD sidecar and substituted into MSL as literal aggregates.
// Metal then indexes them per-sample via the nested-VSArray stride
// lowering exercised by testMetalNestedVSArrays.

const float ODT_PEAK_LUM = 100.0;
...
const ODTParams ODT_PARAMS_REC709 =
    init_ODTParams(ODT_PEAK_LUM, REC709_PRI, REC709_PRI, 1);

// Module-init tables. The reach tables use AP1 primaries (the CAM16
// rendering-space gamut); the cusp + upper-hull gamma tables use the
// output's limiting primaries (Rec.709 here).
const float REACH_GAMUT_TABLE[totalTableSize][3] =
    make_gamut_table(AP1, ODT_PEAK_LUM);

const float REACHM_TABLE[gamutTableSize] =
    make_reachM_table(AP1, ODT_PARAMS_REC709.limitJmax, ODT_PEAK_LUM);

const float GAMUT_CUSP_TABLE[totalTableSize][3] =
    make_gamut_table(REC709_PRI, ODT_PEAK_LUM);

const float GAMUT_TOP_GAMMA[totalTableSize] =
    make_upper_hull_gamma_table(GAMUT_CUSP_TABLE, ODT_PARAMS_REC709, ODT_PEAK_LUM);
          ...
          output varying float bOut)
{
    ...
    float JMh[3] = aces_to_JMh(aces, ODT_PEAK_LUM);
    ...
    float XYZ[3] = JMh_to_output_XYZ(compressedJMh, ODT_PARAMS_REC709);
...
// Round-trip entry: aces -> fwd pipeline -> inverse pipeline -> aces.
// Bypasses the display XYZ endpoint so the inverse path tests tonemap
// and gamut compression in isolation rather than the XYZ<->JMh wrapper.
void roundtrip_main(input varying float rIn,
                    input varying float gIn,
                    input varying float bIn,
                    output varying float rOut,
                    output varying float gOut,
                    output varying float bOut)
{
    float aces_in[3] = {rIn, gIn, bIn};

    float JMh[3] = aces_to_JMh(aces_in, ODT_PEAK_LUM);
    ...
    float uncompressedJMh[3] = gamutMap_inv(compressedJMh,
                                            ODT_PARAMS_REC709,
                                            GAMUT_CUSP_TABLE,
                                            GAMUT_TOP_GAMMA,
                                            REACH_GAMUT_TABLE,
    ...
    float untonemappedJMh[3] = tonemapAndCompress_inv(uncompressedJMh,
                                                      ODT_PARAMS_REC709,
                                                      REACH_GAMUT_TABLE,
                                                      REACHM_TABLE);

    float aces_out[3] = JMh_to_aces(untonemappedJMh, ODT_PEAK_LUM);
